...
def train(options: ESDConfig):
    ...
    logger = WandbLogger(
        ...
        project="cs-175-final-project",
        config={"architecture": options.model_type, "learning_rate": options.learning_rate},
    )
    ...
    #esd_datamodule.modify_ground_truth()
    ...
    model_params = {
        ...
        "wandb_run_name": options.wandb_run_name
    }
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # initialize the ESDSegmentation module
    esd_segmentation = ESDSegmentation(options.model_type, options.in_channels, options.out_channels, options.learning_rate, model_params)
    esd_segmentation.to(device)
    ...
    trainer.fit(model=esd_segmentation, datamodule=esd_datamodule)
    # run trainer.fit
